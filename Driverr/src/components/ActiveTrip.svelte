<script lang="ts">
  import { activeTripStatus, currentView, currentActiveRide } from '../stores/appStore';
  ...
  function getButtonText() {
    switch($activeTripStatus) {
      case 'Not Started': return 'التوجه لنقطة الالتقاء';
      case 'Heading to Pickup': return 'لقد وصلت لنقطة الالتقاء';
      case 'Arrived at Pickup': return 'بدء الرحلة';
      case 'In Progress': return 'إنهاء الرحلة';
      case 'Completed': return 'العودة للرئيسية';
      default: return 'بدء';
    }
  }
  ...
  function getStatusArabic() {
    switch($activeTripStatus) {
      case 'Not Started': return 'بإنتظار الإنطلاق';
      case 'Heading to Pickup': return 'جاري التوجه للراكب';
      case 'Arrived at Pickup': return 'في موقع الالتقاء';
      case 'In Progress': return 'في الطريق للوجهة';
      case 'Completed': return 'تم إكمال الرحلة';
      default: return '';
    }
  }
  ...
  function getProgressWidth() {
    switch($activeTripStatus) {
      ...
      case 'Heading to Pickup': return '25%';
      case 'Arrived at Pickup': return '50%';
...
<div class="active-trip-container animate-fade container">
  ...
  <div class="trip-info card">
    ...
    <div class="trip-meta">
       ...
       <div class="meta-item">
        <strong>السعر</strong>
        <span>{$currentActiveRide?.price ? $currentActiveRide.price + ' ع.د' : '---'}</span>
      </div>
    </div>
  </div>
  ...
  <div class="control-panel">
    ...
    <button class="btn btn-large {$activeTripStatus === 'Completed' ? 'btn-success' : 'btn-primary'}" on:click={handleAction} disabled={isLoading}>
      {isLoading ? 'جاري التحميل...' : getButtonText()}
      {#if !isLoading}
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="transform: scaleX(-1);"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
      {/if}
    </button>
  </div>
</div>
...
<style>
  ...
  @keyframes pulse {
    0%   { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(250, 196, 69, 0.7); }
    70%  { transform: scale(1);    box-shadow: 0 0 0 6px rgba(250, 196, 69, 0); }
    100% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(250, 196, 69, 0); }
  }
  ...
  .p-details h4 { margin: 0; font-size: 1rem; }
  .p-details span { font-size: 0.8rem; color: var(--text-muted); }
  ...
  .timeline-dot.pickup { border-color: var(--primary); }
  .timeline-dot.dropoff { border-color: var(--success); }
